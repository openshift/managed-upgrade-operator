...
var _ = Describe("ClusterUpgrader maintenance window tests", func() {
	...
	Context("When removing a control plane maintenance window", func() {
		It("Asks the maintenance client to do so", func() {
			...
			result, err := RemoveControlPlaneMaintWindow(mockKubeClient, mockScaler, mockMetricsClient, mockMaintClient, upgradeConfig, logger)
	...
	Context("When creating a control plane maintenance window", func() {
		It("Asks the maintenance client to do so", func() {
			...
			result, err := CreateControlPlaneMaintWindow(mockKubeClient, mockScaler, mockMetricsClient, mockMaintClient, upgradeConfig, logger)
			Expect(err).NotTo(HaveOccurred())
			Expect(result).To(BeTrue())
		})
		It("Indicates when creating the maintenance window has failed", func() {
	...
	Context("When creating a worker maintenance window", func() {
		...
		It("Asks the maintenance client to do so", func() {
			...
			result, err := CreateWorkerMaintWindow(mockKubeClient, mockScaler, mockMetricsClient, mockMaintClient, upgradeConfig, logger)
			Expect(err).NotTo(HaveOccurred())
			Expect(result).To(BeTrue())
		})
		It("Indicates when creating the maintenance window has failed", func() {
	...
	Context("When removing a worker maintenance window", func() {
		It("Asks the maintenance client to do so", func() {
			mockMaintClient.EXPECT().End().Times(1)
			result, err := RemoveMaintWindow(mockKubeClient, mockScaler, mockMetricsClient, mockMaintClient, upgradeConfig, logger)
			Expect(err).NotTo(HaveOccurred())
			Expect(result).To(BeTrue())
		})
		It("Indicates when creating the maintenance window has failed", func() {
