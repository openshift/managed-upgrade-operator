...
Yes, if `capacityReservation` is not disabled in the upgradeconfig CR. MUO will create a temporary +1 to every worker `machinesets` within the cluster. In multi availability zones, this is true for each zone.

**Does MUO maintain correct instance types for each machine pool?**

Yes, if `capacityReservation` is not disabled in the upgradeconfig CR. MUO creates the extra compute based on the found instance types of the current `machinesets`.
