...
var _ = Describe("UpgradeConfigController", func() {
	...
	It("forwards the supplied context to the upgrader and status update", func() {
		...
		gomock.InOrder(
			mockClusterUpgrader.EXPECT().UpgradeCluster(ctx, upgradeConfig, log).Return(upgradev1alpha1.UpgradePhaseUpgrading, nil),
			mockKubeClient.EXPECT().Status().Return(mockUpdater),
			mockUpdater.EXPECT().Update(ctx, upgradeConfig).Return(nil),
		)

		result, err := reconciler.upgradeCluster(ctx, mockClusterUpgrader, upgradeConfig, log)
		Expect(err).NotTo(HaveOccurred(), "forwarding the context should preserve a successful reconciliation")
		Expect(result.RequeueAfter).To(Equal(upgradingReconcileTime), "an ongoing upgrade should retain its requeue interval")
	})
		...
		Context("When attempting to fetch the configmap", func() {
			...
			It("The configuration configmap must exist", func() {
				gomock.InOrder(
					...
					mockCVClient.EXPECT().GetClusterVersion(gomock.Any()).Return(testClusterVersion, nil),
