...
```
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_managed_upgrade_operator/<PR_NUMBER>/pull-ci-openshift-managed-upgrade-operator-master-lint/<BUILD_ID>
```
...
**Job History**: https://prow.ci.openshift.org/?repo=openshift%2Fmanaged-upgrade-operator
...
```bash
# View recent PR jobs
curl -s "https://prow.ci.openshift.org/?repo=openshift%2Fmanaged-upgrade-operator&type=presubmit" | grep -E "pull-ci-openshift-managed-upgrade-operator"
...
- **Pull request jobs**: `gs://test-platform-results/pr-logs/pull/openshift_managed_upgrade_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]`
- **Periodic jobs**: `gs://test-platform-results/logs/[JOB_NAME]/[JOB_ID]`

**Viewing logs via web**:
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_managed_upgrade_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]
```
...
- `pull-ci-openshift-managed-upgrade-operator-master-e2e-binary-build-success` - E2E binary build verification
- `pull-ci-openshift-managed-upgrade-operator-master-coverage` - Code coverage analysis (with Codecov)
- `pull-ci-openshift-managed-upgrade-operator-master-lint` - Linting checks
- `pull-ci-openshift-managed-upgrade-operator-master-test` - Unit tests
- `pull-ci-openshift-managed-upgrade-operator-master-validate` - Validation checks

**Tekton Pipelines** (configured in `.tekton/`):
- `managed-upgrade-operator-pull-request` - Main PR pipeline (docker build with OCI-TA)
- `managed-upgrade-operator-e2e-pull-request` - E2E testing pipeline
- `managed-upgrade-operator-pko-pull-request` - PKO (Package Operator) pipeline
...
```text
https://prow.ci.openshift.org/?repo=openshift%2Fmanaged-upgrade-operator&type=presubmit
```
...
- Builds container images to Quay (managed-upgrade-operator-tenant)
...
# Option 2: Use gh CLI to find URLs
gh pr checks <PR_NUMBER> | grep "pull-ci-openshift-managed-upgrade-operator"
...
# https://prow.ci.openshift.org/?repo=openshift%2Fmanaged-upgrade-operator

# Or use platform-specific command:
# macOS: open "https://prow.ci.openshift.org/?repo=openshift%2Fmanaged-upgrade-operator"
# Linux: xdg-open "https://prow.ci.openshift.org/?repo=openshift%2Fmanaged-upgrade-operator"
# Windows: start "https://prow.ci.openshift.org/?repo=openshift%2Fmanaged-upgrade-operator"

# View specific PR on Prow (replace <PR_NUMBER>)
# https://prow.ci.openshift.org/?repo=openshift%2Fmanaged-upgrade-operator&type=presubmit&pull=<PR_NUMBER>
```
...
- Prow jobs: `pull-ci-openshift-managed-upgrade-operator-master-*`
- Tekton jobs: Usually show as "Tekton" or pipeline names in PR checks
- Verify repo name format in Prow: `openshift_managed_upgrade_operator` (underscore, not dash)
...
- Common issues:
  ...
  - Auth failures → Secret configuration in `managed-upgrade-operator-tenant` namespace
- Local validation:
  ```bash
...
- Location: `ci-operator/config/openshift/managed-upgrade-operator/openshift-managed-upgrade-operator-master.yaml`
- Update process: Submit PR to openshift/release repository
- Auto-generated jobs in: `ci-operator/jobs/openshift/managed-upgrade-operator/`

**Tekton Pipelines** (in this repo):
- Location: `.tekton/` directory
- Files:
  - `managed-upgrade-operator-pull-request.yaml` - Main PR pipeline
  - `managed-upgrade-operator-push.yaml` - Post-merge pipeline
  - `managed-upgrade-operator-e2e-pull-request.yaml` - E2E testing
  - `managed-upgrade-operator-pko-pull-request.yaml` - PKO validation
...
- Secret: `managed-upgrade-operator-codecov-token` (stored in Prow)
- Generate coverage locally: `make coverage`
- Coverage runs on PRs and post-merge (`publish-coverage`)
- Dashboard: Check Codecov for managed-upgrade-operator
